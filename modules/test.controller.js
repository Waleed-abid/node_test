...
const solution = () => {
  ...
  while (calculations.customersRemaining > 0) {
    ...
    if (calculations.prevDistance === 0) {
      ...
      calculations.prevDistance = round(
        calculations.dosfec[closest].distance.value / 1609
      );
      ...
      if (
        ...
        for (let i = 0; i < calculations.nod; i += 1) {
          ...
          if (
            ...
            /* If the distance of the last selected closest customer
                    is greater than or equal to the current customer */
            docfec[closest].distance.value >= docfec[i].distance.value &&
            /* And of the the current customer is not allocated a route */
            calculations.dosfec[i].allocated !== true
          ) {
            /* Set the current customer in the loop to be the closest */
        ...
        const value =
          docfec[closest].distance.value / 1609 + calculations.prevDistance;
        if (value < maxRouteLength) {
          ...
          routeObject.path = `${docfec[closest].distance.text} from ${
            customers[calculations.lastAllocated]
          } to ${customers[closest]}.`;
        ...
        calculations.deliveryRoutes.push({
          totalLength: `${calculations.prevDistance} miles from Store.`,
